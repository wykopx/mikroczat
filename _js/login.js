...
export const loginDialog = document.querySelector("#loginDialog");
...
const showLoginDialogButton = document.querySelector("#showLoginDialogButton");
const closeLoginDialogButton = document.querySelector("#closeLoginDialogButton");
const loginAsGuestButton = document.querySelector("#loginAsGuestButton");
...
export let tokensObject = getTokenObjectFromLocalStorage();
if (dev)
    console.log("1. tokensObject: ", tokensObject);
if ((tokensObject.token || tokensObject.refresh_token)) {
    logIn();
}
export function getTokenObjectFromLocalStorage(username) {
    const tokensObject = {
        token: localStorage.getItem('token') ?? null,
        refresh_token: localStorage.getItem('userKeep') ?? null
    };
    if (dev)
        console.log(`getTokenObjectFromLocalStorage() -> tokensObject: `, tokensObject);
    return tokensObject;
}
export function processLoginData(pastedData) {
...
export async function logIn() {
    ...
    if (tokensObject.refresh_token) {
        let newTokensObject = await api.refreshTokenFromAPI();
        if (dev)
    ...
    if (!tokensObject.token)
        tokensObject = getTokenObjectFromLocalStorage();
    ...
    await fetch(`${CONST.apiPrefixURL}/profile/short`, {
        ...
        if (dev)
            console.log(`loggedUser: ${loggedUser.username}`, loggedUser);
...
export async function confirmLoggedIn() {
    ...
    if (loggedUser?.username != "Gość") {
        ...
        document.querySelectorAll("a.loggedInHref").forEach((el) => {
            ...
            el.classList.add(`${loggedUser.status}`, `${loggedUser.color}-profile`, `${loggedUser.gender}-gender`);
        });
    }
    if (window.opener) {
        if (dev)
            console.log("Sending message to main wykop.pl tab");
    ...
    if (openChannelsFromURLArray.length > 0) {
        for (const channelName of openChannelsFromURLArray) {
            ...
            openedChannels.set(channelName.replaceAll(/_+$/g, ""), newChannel);
        }
    }
    ...
    switch (event.data.type) {
        case "TokensObject":
            if (event.data.userKeep) {
                api.saveTokenInLocalStorage({ tokenValue: event.data.userKeep, tokenType: "userKeep" });
            }
            else if (event.data.token) {
                api.saveTokenInLocalStorage({ tokenValue: event.data.token, tokenType: "token" });
            }
            break;
        case "token":
            if (event.data.token) {
...
window.logout = function () {
    ...
    if (mikroczatLoggedByWykopCzatButton)
        window.close();
    else
...
document.addEventListener('DOMContentLoaded', (DOMContentLoadedEvent) => {
    ...
    setTimeout(function () {
        if (!dev)
            console.clear();
        if (mikroczatLoggedIn && loggedUser && loggedUser.username)
            console.log(`Witaj ${loggedUser.username}, życzę miłego mirkowania na mikro𝗰𝘇𝗮𝗰𝗶𝗲 ( ͡° ͜ʖ ͡°)  `);
        else
            console.log(`Witaj Mireczku lub Mirabelko, życzę miłego mirkowania na mikro𝗰𝘇𝗮𝗰𝗶𝗲 ( ͡° ͜ʖ ͡°)  `);
        console.log(" ");
        if (!mikroczatLoggedIn) {
            if (!mikroczatLoggedIn) {
                loginDialog.showModal();
                ...
                if (ua.browser.is("firefox"))
                    console.log(`https://addons.mozilla.org/pl/firefox/addon/tampermonkey/`);
                else if (ua.browser.is("safari"))
                    console.log(`https://apps.apple.com/pl/app/tampermonkey/id1482490089`);
                else if (ua.browser.is("opera"))
                    console.log(`https://addons.opera.com/pl/extensions/details/tampermonkey-beta/`);
                else if (ua.browser.is("edge"))
                    console.log(`https://microsoftedge.microsoft.com/addons/detail/iikmkjmpaadaobahmlepeloendndfphd`);
                else
                    console.log(`https://chromewebstore.google.com/detail/tampermonkey/dhdgffkkebhmkfjojejmpbldmpobfkfo`);
                ...
                if (ua.browser.is("opera"))
                    console.log("2. Otwórz narzędzia deweloperskie tak jak zrobiłeś/aś to tutaj. Możesz użyć skrótu klawiszy [CTRL] + [SHIFT] + [J]");
                else
                    console.log("2. Otwórz narzędzia deweloperskie tak jak zrobiłeś/aś to tutaj. Możesz użyć klawisza [F12] albo [CTRL] + [SHIFT] + [J]");
                if (ua.browser.is("firefox")) {
