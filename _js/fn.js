...
export function markdownTagsToLink(text, currentChannelName)
{
    return text.replace(/(\B)#([a-zA-Z0-9]+)/g, function (wholeMatch, textInFront, tag)
    {
        ...
        {
            return `${textInFront}<a class="href_channel currentChannel" href="/czat/${tag}">#${tag}</a>`;
        }
        else
        {
            return `${textInFront}<a class="href_channel" href="/czat/${tag}" target="_blank" title='Otwórz kanał "#${tag}" w nowym oknie MikroCzata'>#${tag}</a>`;
        }
    });
}
